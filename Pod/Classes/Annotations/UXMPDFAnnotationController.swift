...
open class UXMPDFAnnotationController: UIViewController {
    ...
    var annotationType: UXMPDFAnnotation.Type?
    
    open var annotationTypes: [UXMPDFAnnotation.Type] = [
        UXMPDFTextAnnotation.self,
    ...
    /// Current annotation
    open var currentAnnotation: UXMPDFAnnotation?
    ...
    lazy var undoButton: PDFBarButton = PDFBarButton(
        ...
        toggled: false,
        target: self,
        action: #selector(UXMPDFAnnotationController.selectedUndo(_:))
    )
     ...
     */
    public init(document: UXMPDFDocument, delegate: PDFAnnotationControllerProtocol) {
    ...
    public convenience init(document: UXMPDFDocument,
                            ...
                            annotationTypes: [UXMPDFAnnotation.Type]) {
    ...
    func annotationBecomeActive(_ sender: Notification) {
        guard let obj = sender.object as? UXMPDFAnnotation else {
            return
        }
    ...
    open func startAnnotation(_ type: UXMPDFAnnotation.Type?) {
        finishAnnotation()
        annotationType = type
...
//        finishAnnotation()
    ...
    func select(annotation: UXMPDFAnnotation?) {
        self.currentAnnotation?.didEnd()
        self.currentAnnotation = annotation
        self.currentAnnotation?.delegate = self
    }
    
    func loadButtons(for annotations: [UXMPDFAnnotation.Type]) {
...
extension UXMPDFAnnotationController: PDFAnnotationEvent {
    public func annotationUpdated(annotation: UXMPDFAnnotation) {  }
    
    public func annotation(annotation: UXMPDFAnnotation, selected action: String) {
        if action == "delete" {
...
extension UXMPDFAnnotationController: PDFRenderer {
