...
open class UXMFormSignatureField: UXMFormField {
    ...
    fileprivate lazy var signImage: UIImageView = {
...
class UXMFormSignatureViewController: UIViewController {
    ...
    fileprivate lazy var clearButton: UIBarButtonItem = UIBarButtonItem(
        title: "Clear",
        style: .plain,
        target: self,
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        signatureView.frame = CGRect(x: 0, y: 0, width: view.frame.width, height: view.frame.height)
    }
    ...
    @IBAction func tappedDone() {
        delegate?.completedSignatureDrawing(field: signatureView)
...
class UXMFormFieldSignatureCaptureView: UIView {
    ...
    public func getPDFSignature() -> Data {
        ...
        guard let dataConsumer = CGDataConsumer(data: mutableData!) else { fatalError() }
